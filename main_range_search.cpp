...
#define TRAIL_MAX_VALUE (2048)
#define AGENT_COUNT (512)
#define TRAIL_DECAY (2)
#define TRAIL_DIFFUSION (0.005)
#define AGENT_TRAIL_STRENGTH_NORMAL (8)
#define AGENT_TRAIL_STRENGTH_HIGH (128)
#define AGENT_TRAIL_HIGH_STRENGTH_THRESHOLD (0)
#define AGENT_SPEED (1.0)
#define AGENT_TURN_ANGLE (0.02)
#define AGENT_RANDOM_TURN_ANGLE (0.0001)
#define AGENT_TRAIL_SEARCH_SIZE (100)
#define AGENT_TRAIL_SEARCH_ANGLE (0.20)
#define AGENT_TRAIL_SEARCH_DISTANCE (3.0)
...
{
    uint32_t agentX = round(agent.x);
    uint32_t agentY = round(agent.y);
    uint32_t trailsX[AGENT_TRAIL_SEARCH_SIZE];
    uint32_t trailsY[AGENT_TRAIL_SEARCH_SIZE];
    uint32_t trailsIndex = 0;
    uint32_t xStart = 0, xEnd = GRID_WIDTH, yStart = 0, yEnd = GRID_HEIGHT;

    // Find bounds of search space
    if (agentX >= AGENT_TRAIL_SEARCH_DISTANCE)
    {
        xStart = agentX - AGENT_TRAIL_SEARCH_DISTANCE;
    }
    if (agentX <= GRID_WIDTH - AGENT_TRAIL_SEARCH_DISTANCE)
    {
        xEnd = agentX + AGENT_TRAIL_SEARCH_DISTANCE;
    }
    if (agentY >= AGENT_TRAIL_SEARCH_DISTANCE)
    {
        yStart = agentY - AGENT_TRAIL_SEARCH_DISTANCE;
    }
    if (agentY <= GRID_HEIGHT - AGENT_TRAIL_SEARCH_DISTANCE)
    {
        yEnd = agentY + AGENT_TRAIL_SEARCH_DISTANCE;
    }

    // Find all trails ahead
    for (uint32_t y = yStart; y < yEnd; y++)
    {
        for (uint32_t x = xStart; x < xEnd; x++)
        {
            double cellAngleDiff = abs(agent.direction - getAngle(agentX, agentY, x, y));
            if (min(cellAngleDiff, -cellAngleDiff + 1) <= AGENT_TRAIL_SEARCH_ANGLE)
            {
                trailsX[trailsIndex] = x;
                trailsY[trailsIndex] = y;
                trailsIndex++;
                if (trailsIndex > AGENT_TRAIL_SEARCH_SIZE)
                {
                    break;
                }
            }
    ...
    uint32_t targetX = GRID_WIDTH;
    uint32_t targetY = GRID_HEIGHT;
    uint32_t targetStrength = 0;
    uint32_t targetCount = 0;
    if (agentX < AGENT_WALL_SAFETY_DISTANCE || agentX > GRID_WIDTH - AGENT_WALL_SAFETY_DISTANCE || agentY < AGENT_WALL_SAFETY_DISTANCE || agentY > GRID_HEIGHT - AGENT_WALL_SAFETY_DISTANCE)
    {
        // Set middle as target
        targetX = GRID_WIDTH / 2;
        targetY = GRID_HEIGHT / 2;
        targetStrength = TRAIL_MAX_VALUE;
        targetCount = 1;
        // targetCount = 0;
        // agent.direction = getAngle(agentX, agentY, static_cast<double>(GRID_WIDTH / 2), static_cast<double>(GRID_HEIGHT / 2));
    }
    else if (trailsIndex > 0)
    {
        // Pick strongest trail
        for (size_t j = 0; j < trailsIndex; j++)
        {
            if (grid->grid[(trailsY[j] * GRID_WIDTH) + trailsX[j]] >= targetStrength)
            {
                if (grid->grid[(trailsY[j] * GRID_WIDTH) + trailsX[j]] == targetStrength)
                {
                    targetCount++;
                ...
                else
                {
                    ...
                    targetX = trailsX[j];
                    targetY = trailsY[j];
                    targetStrength = grid->grid[(trailsY[j] * GRID_WIDTH) + trailsX[j]];
                }
            }
        }
    }
    ...
    if (targetCount > 0)
    {
        ...
        double targetAngle = getAngle(agentX, agentY, static_cast<double>(targetX) / targetCount, static_cast<double>(targetY) / targetCount);

        if (abs(agent.direction - targetAngle) >= 0.5)
        {
            // Faster to loop around
            if (-abs(agent.direction - targetAngle) + 1 < AGENT_TURN_ANGLE)
            {
                agent.direction = targetAngle;
            }
            else
            {
                if (targetAngle < agent.direction)
                {
        ...
        {
            if (abs(agent.direction - targetAngle) < AGENT_TURN_ANGLE)
            {
                agent.direction = targetAngle;
            }
            else
    ...
    // Add agent trail
    if (grid->grid[(static_cast<uint32_t>(agent.y) * GRID_WIDTH) + static_cast<uint32_t>(agent.x)] < AGENT_TRAIL_HIGH_STRENGTH_THRESHOLD)
    {
